...
(function(){
    ...
    window.lgKb = {
        ...
        onKeyboardShow :        //  user's virtual keyboard show event handler
            function (event)
            {
            },
        onKeyboardHide :        //  user's virtual keyboard hide event handler
            function (event)
            {
            },
        keyMouseOver :
...
//                              lgKb.changeLangJs(lgKb.nextLang);       //  this is called HIDLanguageChange event
        ...
        fireMouseOver :
            function (keyId)
            {
                lgKb.fireMouseEvent(keyId, "mouseover");
            },
        fireMouseOut :
            function (keyId)
            {
                lgKb.fireMouseEvent(keyId, "mouseout");
            },
        fireMouseClick :
            function (keyId)
            {
                lgKb.fireMouseEvent(keyId, "click");
            },
        fireMouseDown :
            function (keyId)
            {
                lgKb.fireMouseEvent(keyId, "mousedown");
            },
        fireMouseUp :
            function (keyId)
            {
                lgKb.fireMouseEvent(keyId, "mouseup");
            },
        WindowFocusIn :
            function (event)
            {
               ...
               else if( ( (event.target.tagName=="TEXTAREA") || ( (event.target.tagName=="INPUT") && ( (event.target.type=="text") || (event.target.type=="password") ) ) )
        ...
        isWebOSTv :
            ...
            {
                var userAgent = new String(navigator.userAgent);
                return ((userAgent.search(/LG Browser/i) != -1) && (userAgent.search(/webOS.TV/i) != -1))? true : false;
            },
        initKeyboard :
            function (event)
            {
...
//                      return;     //  Virtual Keyboard works well on LG Smart TV
        ...
        initHID :
            function ()
            {
                ...
                if(window.NetCastHIDInitialize)
                {
                    ...
                    if("onhidlanguagechanged" in window)
                    {
                        window.addEventListener("hidlanguagechanged", function() { lgKb.changeLangJs(lgKb.nextLang); }, true);
                    }
                    else
                    {
        ...
        setDefaultLanguage :
            function ()
            {
                ...
                if(lgKb.isLgBrowser())
                {
                    if((lgVkDevice.tvLanguage2 != null) && (lgKb.getLangCodeFromId(lgVkDevice.tvLanguage2) != ""))
                    {
        ...
        setCaretPosition :
            function (pos, r)
            {
                ...
                {
                    ctrl.setSelectionRange(pos, new Number(pos));
                }
        ...
        deleteSelectedChar :
            function ()
            {
                if(lgKb.targetElement.selectionEnd == lgKb.targetElement.selectionStart)
                {
                    return false;
                }
                ...
                kResult = kTxt.substr(0, lgKb.targetElement.selectionStart) + kTxt.substr(lgKb.targetElement.selectionEnd, kTxt.length);
                lgKb.putStrIntoFld(kResult, lgKb.targetElement.selectionStart - 1);
    ...
    if(lgKb.isWebOSTv())
    {
        lgKb.setCookie = lgKb.setLocalStorage;
        lgKb.getCookie = lgKb.getLocalStorage;
    }
